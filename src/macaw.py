...
class MACAW(object):
    def __init__(self, args: argparse.Namespace,
        ...
        self._adaptation_policy = MLP([self._observation_dim + goal_dim] +
                                      ...
                                      adapt_context_only=args.adapt_context_only,
                                      n_adaptation_layers=args.n_adaptation_layers).to(args.device)
        ...
        if args.cvae:
            self._exploration_policy = CVAE(self._observation_dim + goal_dim, self._action_dim, args.latent_dim,
                                            ...
                                            preprocess=args.cvae_preprocess).to(args.device)
        else:
        ...
        self._q_function = MLP(
            [self._observation_dim + goal_dim + self._action_dim] + [args.net_width] * args.net_depth + [1],
            bias_linear=not args.no_bias_linear,
        ...
        inner_buffers = [task_config.train_buffer_paths.format(idx) if load_inner_buffers else None for idx in
                         task_config.train_tasks]
        outer_buffers = [task_config.train_buffer_paths.format(idx) if load_outer_buffers else None for idx in
                         task_config.train_tasks]
        test_buffers = [task_config.test_buffer_paths.format(idx) if load_test_buffers else None for idx in
                        task_config.test_tasks]

        self._test_buffers = [ReplayBuffer(args.inner_buffer_size, self._observation_dim, env_action_dim(self._env),
                                           discount_factor=discount_factor,
                                           immutable=test_buffers[i] is not None, load_from=test_buffers[i],
                                           silent=silent, skip=args.inner_buffer_skip,
        ...
        if not self._args.online_ft:
            self._inner_buffers = [
                ReplayBuffer(args.inner_buffer_size, self._observation_dim, env_action_dim(self._env),
                             discount_factor=discount_factor,
            ...
            if args.offline and args.load_inner_buffer and args.load_outer_buffer and (
                    args.replay_buffer_size == args.inner_buffer_size) and (
                    args.buffer_skip == args.inner_buffer_skip) and args.buffer_mode == 'end':
                self._outer_buffers = self._inner_buffers
            else:
                self._outer_buffers = [
                    ReplayBuffer(args.replay_buffer_size, self._observation_dim, env_action_dim(self._env),
        ...
        if self._policy_lrs is None:
            self._policy_lrs = [torch.nn.Parameter(
                torch.tensor(float(np.log(self._args.inner_policy_lr)) if not self._args.multitask else 10000.).to(
                    args.device))
                                for p in self._adaptation_policy.adaptation_parameters()]
            self._value_lrs = [torch.nn.Parameter(
                torch.tensor(float(np.log(self._args.inner_value_lr)) if not self._args.multitask else 10000.).to(
                    args.device))
                               for p in self._value_function.adaptation_parameters()]
            self._q_lrs = [torch.nn.Parameter(
                torch.tensor(float(np.log(self._args.inner_value_lr)) if not self._args.multitask else 10000.).to(
                    args.device))
                           for p in self._q_function.adaptation_parameters()]
            if args.advantage_head_coef is not None:
                self._adv_coef = torch.nn.Parameter(
                    torch.tensor(float(np.log(args.advantage_head_coef))).to(args.device))
    ...
    #################################################################
    # @profile
    def _rollout_policy(self, policy: MLP, env, sample_mode: bool = False, random: bool = False,
                        render: bool = False) -> List[Experience]:
        ...
        while not done:
            ...
            if not random:
                with torch.no_grad():
                    ...
                    log_prob = D.Normal(mu, torch.empty_like(mu).fill_(self._action_sigma)).log_prob(action).to(
                        self._cpu).numpy().sum()
                    action = action.squeeze().to(self._cpu).numpy().clip(min=env.action_space.low,
    ...
    def mc_value_estimates_on_batch(self, value_function, batch, task_idx, no_bootstrap=False):
        ...
        if not no_bootstrap:
            terminal_state_value_estimates = value_function(self.add_task_description(
                batch[:, self._observation_dim * 2 + self._action_dim:self._observation_dim * 3 + self._action_dim],
                task_idx))
            bootstrap_correction = batch[:,
                                   -4:-3] * terminal_state_value_estimates  # I know this magic number indexing is heinous... I'm sorry
    ...
    def value_function_loss_on_batch(self, value_function, batch, inner: bool = False, task_idx: int = None,
                                     iweights: torch.tensor = None, target=None):
        value_estimates = value_function(self.add_task_description(batch[:, :self._observation_dim], task_idx))
        with torch.no_grad():
            if target is None:
                target = value_function
            mc_value_estimates = self.mc_value_estimates_on_batch(target, batch, task_idx, self._args.no_bootstrap and (
                        inner or self._args.multitask))
    ...
    # @profile
    def adaptation_policy_loss_on_batch(self, policy, q_function, value_function, batch, task_idx: int,
                                        inner: bool = False,
                                        iweights: torch.tensor = None, online: bool = False):
        with torch.no_grad():
            value_estimates = value_function(self.add_task_description(batch[:, :self._observation_dim], task_idx))
            if q_function is not None:
                action_value_estimates = q_function(torch.cat((batch[:, :self._observation_dim], batch[:,
                                                                                                 self._observation_dim:self._observation_dim + self._action_dim]),
                                                              -1))
            else:
            ...
            if self._args.no_norm:
                ...
                normalized_advantages = (1 / self._adaptation_temperature) * (
        ...
        action_log_probs = action_distribution.log_prob(
            batch[:, self._observation_dim:self._observation_dim + self._action_dim]).sum(-1)
    ...
    def update_model(self, model: nn.Module, optimizer: torch.optim.Optimizer, clip: float = None,
    ...
    def update_params(self, params: list, optimizer: torch.optim.Optimizer, clip: float = None,
                      extra_grad: list = None):
        optimizer.step()
        optimizer.zero_grad()
    ...
    def soft_update(self, source, target):
        for param_source, param_target in zip(source.named_parameters(), target.named_parameters()):
            ...
            param_target[1].data = self._args.target_vf_alpha * param_target[1].data + (
    ...
    def eval_multitask(self, train_step_idx: int, writer: SummaryWriter):
        ...
        for i, (test_task_idx, test_buffer) in enumerate(zip(self.task_config.test_tasks, self._test_buffers)):
            ...
            if self._args.eval:
                adapted_trajectory, adapted_reward, success = self._rollout_policy(self._adaptation_policy, self._env,
                                                                                   sample_mode=True,
    ...
    def eval_macaw(self, train_step_idx: int, writer: SummaryWriter, ft: str = 'offline',
        ...
        for i, (test_task_idx, test_buffer) in enumerate(
            ...
            with higher.innerloop_ctx(value_function, opt,
                                      override={'lr': [F.softplus(l) for l in self._value_lrs]}) as (
            f_value_function, diff_value_opt):
                if not self._args.imitation:
                    loss, _, _, _ = self.value_function_loss_on_batch(f_value_function, value_batch,
                                                                      task_idx=test_task_idx, inner=True,
                                                                      target=vf_target)
                ...
                with higher.innerloop_ctx(policy, policy_opt,
                                          override={'lr': [F.softplus(l) for l in self._policy_lrs]}) as (
                f_policy, diff_policy_opt):
                    if self._args.imitation:
                        loss = self.imitation_loss_on_batch(f_policy, policy_batch, None, inner=True)
                    else:
                        loss, _, _, _ = self.adaptation_policy_loss_on_batch(f_policy, None, f_value_function,
                                                                             policy_batch, test_task_idx, inner=True)
                    diff_policy_opt.step(loss)

                    adapted_trajectory, adapted_reward, success = self._rollout_policy(f_policy, self._env,
                                                                                       sample_mode=True,
                                                                                       render=self._args.render)
                    trajectories.append(adapted_trajectory)
                ...
                if not self._args.imitation and ft != "offline":
                    ...
                    for eval_step in range(ft_steps):
                        ...
                        if eval_step > 0:
                            ...
                            print(self._instance_idx, eval_step, eval_rewards, adapted_reward, step_time,
                        ...
                        for traj_idx in range(3):
                            _, adapted_reward, success = self._rollout_policy(adapted_policy, self._env,
                                                                              sample_mode=True,
                                                                              render=self._args.render)
                        ...
                        if ft == 'online':
                            noise_trajectory, _, _ = self._rollout_policy(adapted_policy, self._env, sample_mode=False,
                                                                          render=self._args.render)
                        ...
                        if replay_buffer._stored_steps >= self._args.eval_batch_size:
                            if eval_step > burn_in:
                                for _ in range(steps_per_rollout):
                                    ft_value_batch = torch.tensor(replay_buffer.sample(self._args.eval_batch_size),
                                                                  requires_grad=False).to(self._device)

                                    loss, _, _, _ = self.value_function_loss_on_batch(adapted_vf, ft_value_batch,
                                                                                      task_idx=test_task_idx,
                                                                                      inner=True, target=None)
                                    loss.backward()
                                    ft_v_opt.step()
                                    ft_v_opt.zero_grad()
                                if ft == 'online':
                            ...
                            if eval_step > burn_in:
                                for _ in range(steps_per_rollout * 2):
                                    ft_policy_batch = torch.tensor(replay_buffer.sample(self._args.eval_batch_size),
                                                                   requires_grad=False).to(self._device)
                                    if self._args.imitation:
                                        policy_loss = self.imitation_loss_on_batch(adapted_policy, ft_policy_batch,
                                                                                   None, inner=True)
                                    else:
                                        policy_loss, _, _, _ = self.adaptation_policy_loss_on_batch(adapted_policy,
                                                                                                    None, adapted_vf,
                                                                                                    ft_policy_batch,
                                                                                                    test_task_idx)
                                    ...
                                    ft_p_opt.zero_grad()
                                if ft == 'online':
                                    writer.add_scalar(f'FTStep_Policy_Loss/Task_{test_task_idx}', policy_loss.item(),
                                                      eval_step)
    ...
    def train_step(self, train_step_idx: int, writer: Optional[SummaryWriter] = None):
        ...
        for i, (train_task_idx, inner_buffer, outer_buffer) in enumerate(
            ...
            value_batch = torch.tensor(
                inner_buffer.sample(self._args.inner_batch_size, contiguous=self._args.contiguous),
                requires_grad=False).to(self._device)
            ...
            if self._args.multitask:
                ...
                meta_value_function_loss, value, mc, mc_std = self.value_function_loss_on_batch(self._value_function,
                                                                                                meta_batch,
                ...
                meta_policy_loss, outer_adv, outer_weights_, _ = self.adaptation_policy_loss_on_batch(
                    self._adaptation_policy, None,
                ...
                if train_step_idx % self._gradient_steps_per_iteration == 0:
                    adapted_trajectory, adapted_reward, success = self._rollout_policy(self._adaptation_policy,
                                                                                       self._env,
                ...
                if True:
                    if not self._args.imitation:
                        ...
                        with higher.innerloop_ctx(vf, opt, override={'lr': [F.softplus(l) for l in self._value_lrs]},
                                                  copy_initial_weights=False) as (f_value_function, diff_value_opt):
                            if len(self._env.tasks) > 1:
                                for step in range(self._maml_steps):
                                    DEBUG(f'################# VALUE STEP {step} ###################', self._args.debug)
                                    sub_batch = value_batch.view(self._args.maml_steps,
                                                                 value_batch.shape[0] // self._args.maml_steps,
                                                                 *value_batch.shape[1:])[step]
                                    loss, value_inner, mc_inner, mc_std_inner = self.value_function_loss_on_batch(
                                        f_value_function, sub_batch, inner=True, task_idx=train_task_idx,
                                        target=vf_target)  # , iweights=iweights_no_action_)
                            ...
                            meta_value_function_loss, value, mc, mc_std = self.value_function_loss_on_batch(
                                f_value_function, meta_batch, task_idx=train_task_idx, target=vf_target)
                            total_vf_loss = meta_value_function_loss / len(self.task_config.train_tasks)
                            if self._args.value_reg > 0:
                                total_vf_loss = total_vf_loss + self._args.value_reg * self._value_function(
                                    value_batch[:, :self._observation_dim]).pow(2).mean()
                            total_vf_loss.backward()
                    ...
                    with higher.innerloop_ctx(self._adaptation_policy, opt,
                                              override={'lr': [F.softplus(l) for l in self._policy_lrs]},
                                              copy_initial_weights=False) as (f_adaptation_policy, diff_policy_opt):
                        if len(self._env.tasks) > 1:
                            for step in range(self._maml_steps):
                                DEBUG(f'################# POLICY STEP {step} ###################', self._args.debug)
                                sub_batch = \
                                policy_batch.view(self._args.maml_steps, policy_batch.shape[0] // self._args.maml_steps,
                                                  *policy_batch.shape[1:])[step]
                                if self._args.imitation:
                                    loss = self.imitation_loss_on_batch(f_adaptation_policy, sub_batch, train_task_idx,
                                                                        inner=True)
                                else:
                                    loss, adv, weights, adv_loss = self.adaptation_policy_loss_on_batch(
                                        f_adaptation_policy, adapted_q_function,
                        ...
                        if self._args.imitation:
                            meta_policy_loss = self.imitation_loss_on_batch(f_adaptation_policy, policy_meta_batch,
                                                                            train_task_idx)
                        else:
                            meta_policy_loss, outer_adv, outer_weights_, _ = self.adaptation_policy_loss_on_batch(
                                f_adaptation_policy, adapted_q_function,
                        ...
                        # Sample adapted policy trajectory, add to replay buffer i [L12]
                        if train_step_idx % self._gradient_steps_per_iteration == 0:
                            adapted_trajectory, adapted_reward, success = self._rollout_policy(f_adaptation_policy,
                                                                                               self._env,
                                                                                               sample_mode=self._args.offline)
                            train_rewards.append(adapted_reward)
                            successes.append(success)
                            ...
                            if not (self._args.offline or self._args.offline_inner):
                                if self._args.sample_exploration_inner:
                                    exploration_trajectory, _, _ = self._rollout_policy(self._exploration_policy,
                                                                                        self._env, sample_mode=False)
            ...
            if train_step_idx % self._gradient_steps_per_iteration == 0:
                if len(inner_value_losses):
                    ...
                    writer.add_scalar(f'Loss_Value_Inner/Task_{train_task_idx}', np.mean(inner_value_losses),
                                      train_step_idx)
                    writer.add_scalar(f'Loss_Policy_Inner/Task_{train_task_idx}', np.mean(inner_policy_losses),
                                      train_step_idx)
                    if len(adv_policy_losses):
                        writer.add_scalar(f'Loss_Policy_Adv_Inner/Task_{train_task_idx}', np.mean(adv_policy_losses),
                                          train_step_idx)
                    writer.add_scalar(f'Value_Mean_Inner/Task_{train_task_idx}', np.mean(inner_values), train_step_idx)
                    writer.add_scalar(f'Advantage_Mean_Inner/Task_{train_task_idx}', np.mean(inner_advantages),
                                      train_step_idx)
                    writer.add_scalar(f'Weight_Mean_Inner/Task_{train_task_idx}', np.mean(inner_weights),
                                      train_step_idx)
                ...
                writer.add_scalar(f'Advantage_Mean_Outer/Task_{train_task_idx}', np.mean(outer_advantages),
                                  train_step_idx)
                ...
                writer.add_scalar(f'Loss_Policy_Outer/Task_{train_task_idx}', np.mean(meta_policy_losses),
                                  train_step_idx)
            ...
            if not self._args.online_ft:
                ...
                for i, (inner_buffer, outer_buffer) in enumerate(zip(self._inner_buffers, self._outer_buffers)):
                    while inner_buffer._stored_steps < self._args.initial_interacts:
                        ...
                        trajectory, reward, success = self._rollout_policy(behavior_policy, self._env,
            ...
            if not self._args.load_inner_buffer:
                for i, test_buffer in enumerate(self._test_buffers):
                    while test_buffer._stored_steps < self._args.initial_test_interacts:
                        ...
                        random_trajectory, _, _ = self._rollout_policy(behavior_policy, self._env,
                                                                       random=self._args.random)
